...
- `${CLAUDE_PLUGIN_ROOT}/skills/securability-verification/SKILL.md` — loaded to read the securable contract and produce per-requirement verdicts on `implemented` claims. Report a verdict (met, not met, not assessable) with evidence for each claim; recommend verification-engineer for execution-based proof and the `verified` status flip. Load and follow its contract verification procedure.
...
4. Score each of the ten SSEM attributes following the review skill's rubric and anchor points, citing specific file paths or patterns.
5. Compute the overall score per the skill's formula. Show the math. Report pillar means as diagnostics only.
6. Tag every finding systemic or local. Use the pattern tag reference from the review skill. Classify severity in attribute points. One systemic finding per shared root cause, never one finding per instance — per the Actionable Security Intelligence Principle (S6.2, SA.4).
7. For each `implemented` claim, run the repo's existing test suite or opengrep rules when they cover the acceptance criterion. Report the verdict with evidence. Recommend verification-engineer for execution-based proof and the `verified` status flip.
...
9. Assemble the three-part report using `templates/report.md` as the scaffold, led by the SA.4 framing line and the machine-readable score block. Persist the report to `<report_dir>/<YYYY-MM-DD>-<scope>.md` when policy names a `report_dir` or the user asks.
...
- **Part 2 -- Detailed Findings**: per-pillar strengths, weaknesses, and recommendations in `templates/finding.md` shape. Escalations routed to boundary-mapper. Requirement verdicts for `implemented` claims.
