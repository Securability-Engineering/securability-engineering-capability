...
description: >-
  The fix half of triage-and-fix: takes a confirmed securability finding and
  produces a minimal, review-ready patch — one root cause, one diff, one test,
  one PR body section. Use when the user asks to "fix this finding",
  "remediate", "patch this", "address this securability issue", "apply the fix
  from the review", "make a PR for this finding", "fix the Isolated Integrity
  violation", "address the unbounded external call", or "patch the mass
  assignment". Also triggers when a merge-steward or triage-analyst report
  names a finding and the user asks to act on it. Do NOT use for finding
...
- `${CLAUDE_PLUGIN_ROOT}/skills/securability-remediation/SKILL.md` — the primary skill. Defines the seven-step procedure (confirm, root-cause, minimal change, test, verify, contract update, PR body), the PR body section template, the quality checklist, and the never list. Load and follow; it is authoritative for the procedure.
- `${CLAUDE_PLUGIN_ROOT}/skills/securability-engineering/SKILL.md` — loaded for the Anti-Pattern Tag Reference and the correct shapes that replace each anti-pattern. When the remediation skill's Step 2 says to identify the correct shape from this table, load the generation skill and consult its anti-pattern tag reference. Do not restate the table; reference the skill.
- `${CLAUDE_PLUGIN_ROOT}/skills/fiasse-lookup/SKILL.md` — loaded to answer FIASSE/SSEM definition questions that arise during remediation. Load and follow; it is authoritative for definitions and section lookups.
