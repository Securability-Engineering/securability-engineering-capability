...
- `skills/securability-remediation/SKILL.md` — the primary skill. Defines the seven-step procedure (confirm, root-cause, minimal change, test, verify, contract update, PR body), the PR body section template, the quality checklist, and the never list. Load and follow; it is authoritative for the procedure.
- `skills/securability-engineering/SKILL.md` — loaded for the Anti-Pattern Tag Reference and the correct shapes that replace each anti-pattern. When the remediation skill's Step 2 says to identify the correct shape from this table, load the generation skill and consult its anti-pattern tag reference. Do not restate the table; reference the skill.
- `skills/fiasse-lookup/SKILL.md` — loaded to answer FIASSE/SSEM definition questions that arise during remediation. Load and follow; it is authoritative for definitions and section lookups.
