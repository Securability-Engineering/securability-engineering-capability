...
- `skills/fiasse-adoption/SKILL.md` — the primary skill. Defines the readiness table, degraded-mode paths, leading and lagging indicator computation, standards-integration edits, role views, and the framework-vs-adoption diagnostic. Load and follow; it is authoritative for the procedure.
- `skills/fiasse-lookup/SKILL.md` — loaded to answer FIASSE/SSEM definition questions that arise during assessment. Load and follow; it is authoritative for definitions and section lookups.
